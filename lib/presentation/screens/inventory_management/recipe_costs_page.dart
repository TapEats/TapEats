...
class _RecipeCostsPageState extends State<RecipeCostsPage> {
        ...
        if (mounted) {
          final shouldCreate = await showDialog<bool>(
            ...
            builder: (context) => AlertDialog(
              ...
              title: const Text('Create Recipe Tables?',
          ...
          try {
            ...
            if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text(
                      'Failed to create recipe tables. Please contact your administrator.'),
  ...
  Future<void> _fetchRecipes() async {
    try {
      ...
      try {
        ...
        final recipesResult = await _supabase.from('recipes').select('''
              recipe_id,
              name,
              yield_quantity,
              yield_unit,
        ...
        for (var recipe in recipesResult) {
          ...
          try {
            final ingredientsResult =
                await _supabase.from('recipe_ingredients').select('''
                  ingredient_id,
                  quantity,
                  unit,
                  notes,
      ...
      if (searchText.isEmpty) {
        ...
        _filteredRecipes = _recipes.where((recipe) {
          ...
          if (recipe['menu_id'] is Map) {
            menuItemName =
                recipe['menu_id']['name']?.toString().toLowerCase() ?? '';
          }

          return recipeName.contains(searchText) ||
              menuItemName.contains(searchText);
        }).toList();
      }
    });
  }
  ...
  double _calculateRecipeCost(String recipeId) {
    ...
    for (var ingredient in ingredients) {
      ...
      if (ingredient['inventory_id'] is Map) {
        unitPrice = (ingredient['inventory_id']['price_per_unit'] as num?)
                ?.toDouble() ??
            0;
      }
                  ...
                  : _filteredRecipes.isEmpty
                      ...
                      : ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            final cost =
                                _calculateRecipeCost(recipe['recipe_id']);
                            final margin = _calculateProfitMargin(
                                recipe['recipe_id'], cost);
  ...
  Widget _buildRecipeCard(
      Map<String, dynamic> recipe, double cost, double margin) {
                      ...
                      if (ingredients.isEmpty)
                        ...
                        Column(
                          children: ingredients.map((ingredient) {
                            ...
                            if (ingredient['inventory_id'] is Map) {
                              itemName = ingredient['inventory_id']
                                      ['item_name'] ??
                                  'Unknown Item';
                              if (unit.isEmpty) {
                                unit = ingredient['inventory_id']['unit'] ?? '';
                              }
                              unitPrice = (ingredient['inventory_id']
                                          ['price_per_unit'] as num?)
                                      ?.toDouble() ??
                                  0;
                            }
                    ...
                    children: [
                      TextButton.icon(
                        ...
                        icon: const Icon(Iconsax.edit,
                            color: Color(0xFFD0F0C0), size: 16),
                        label: const Text(
                          "Edit Recipe",
                          style: TextStyle(color: Color(0xFFD0F0C0)),
                        ),
                            ...
                            children: [
                              Expanded(
                                child: TextFormField(
                                  ...
                                  decoration:
                                      _inputDecoration('Yield Quantity'),
                                  keyboardType:
                            ...
                            children: [
                              Expanded(
                                child: TextFormField(
                                  ...
                                  decoration:
                                      _inputDecoration('Prep Time (min)'),
                              ...
                              Expanded(
                                child: TextFormField(
                                  ...
                                  decoration:
                                      _inputDecoration('Cook Time (min)'),
                                  keyboardType: TextInputType.number,
                                  style: const TextStyle(color: Colors.white),
                                ),
                              ),
                          ...
                          if (ingredients.isEmpty)
                            ...
                            ListView.builder(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                final inventoryItem = ingredient['inventory_id']
                            ...
                            onPressed: nameController.text.trim().isEmpty ||
                                ...
                                : () {
                                    ...
                                    _saveRecipe(
                                      ...
                                      yieldQuantity: double.tryParse(
                                              yieldQtyController.text) ??
                                          1,
                                      yieldUnit:
  ...
  void _showEditRecipeDialog(
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content:
            Text('Edit recipe functionality will be added in a future update'),
  ...
  Future<void> _saveRecipe({
    ...
    try {
      ...
      for (var ingredient in ingredients) {
        final inventoryItem =
            ingredient['inventory_id'] as Map<String, dynamic>;
