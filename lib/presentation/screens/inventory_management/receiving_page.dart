...
class _ReceivingPageState extends State<ReceivingPage> {
      ...
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: const Text(
                  'Purchase orders system is not set up. Please set up purchase orders first.'),
  ...
  Future<void> _fetchPendingOrders() async {
    try {
      ...
      for (var order in ordersResult) {
        ...
        final itemsResult =
            await _supabase.from('purchase_order_items').select('''
              item_id,
              quantity,
              unit_price,
              notes,
      ...
      if (searchText.isEmpty) {
        ...
        _filteredOrders = _pendingOrders.where((order) {
          ...
          if (order['supplier_id'] is Map) {
            supplierName = order['supplier_id']['company_name']
                    ?.toString()
                    .toLowerCase() ??
                '';
          }
  ...
  Widget _buildReceivingItemRow(
    ...
    final noteController =
        TextEditingController(text: receivedNotes[inventoryId] ?? '');
            ...
            children: [
              ...
              SizedBox(
                ...
                child: TextFormField(
                  ...
                  onChanged: (value) {
                    setState(() {
                      receivedQuantities[inventoryId] =
                          double.tryParse(value) ?? orderedQuantity;
                    });
                  },
                ),
              ),
  ...
  Future<void> _processOrderReceiving(
    ...
    try {
      ...
      await _supabase.from('purchase_orders').update({
        'status': 'Received',
      ...
      for (var item in items) {
        ...
        try {
          await _supabase.from('inventory_transactions').insert({
            ...
            'notes': receivedNotes[inventoryId] ??
                'Received from order ${order['purchase_order_id']}',
            'user_id': user.id,
