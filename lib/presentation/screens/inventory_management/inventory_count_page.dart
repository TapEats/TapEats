...
class _InventoryCountPageState extends State<InventoryCountPage> {
  ...
  String _countName =
      'Inventory Count ${DateFormat('MMM d, yyyy').format(DateTime.now())}';
    ...
    try {
      ...
      setState(() {
        ...
        _filteredItems = _filterItems(
            _inventoryItems, _selectedCategory, _searchController.text);
  ...
  List<Map<String, dynamic>> _filterItems(
      List<Map<String, dynamic>> items, String category, String searchText) {
  ...
  void _selectCategory(String category) {
    setState(() {
      ...
      _filteredItems =
          _filterItems(_inventoryItems, category, _searchController.text);
    });
  }
  ...
  Widget _buildInventoryItem(Map<String, dynamic> item) {
    ...
    final hasDifference =
        diff.abs() > 0.001; // Small threshold for floating point errors
                  ...
                  ActionChip(
                    ...
                    onPressed: () {
                      ...
                      countController.text =
                          (current - 1 >= 0 ? current - 1 : 0).toString();
                    },
                  ),
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              onPressed: () {
                // Save count
                final count =
                    double.tryParse(countController.text) ?? systemQty;
  ...
  Future<void> _startCountSession({
    ...
    try {
      ...
      if (existingSession != null) {
        ...
        await _supabase.from('inventory_count_sessions').update({
          'name': name,
        ...
        if (mounted) {
          final shouldCreate = await showDialog<bool>(
            ...
            builder: (context) => AlertDialog(
              backgroundColor: const Color(0xFF222222),
              title: const Text('Create Inventory Count Tables?',
                  style: TextStyle(color: Colors.white)),
              content: const Text(
          ...
          try {
            ...
            if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text(
                      'Failed to create inventory count tables. Please contact your administrator.'),
                  backgroundColor: Colors.red,
                ),
              );
            }
  ...
  Future<void> _saveCountValue(
      String inventoryId, double count, String notes) async {
    try {
      // Update local state
      setState(() {
      ...
      if (_countSessionId.isNotEmpty) {
        try {
          ...
          if (existingItem != null) {
            ...
            await _supabase.from('inventory_count_items').update({
            ...
            // Create new count
            await _supabase.from('inventory_count_items').insert({
            ...
            try {
              ...
              if (diff.abs() > 0.001) {
                await _supabase.from('inventory_transactions').insert({
                  ...
                  'quantity': diff,
                  'notes':
                      'Inventory count adjustment from ${systemQty} to ${countQty}',
                  'user_id': user.id,
