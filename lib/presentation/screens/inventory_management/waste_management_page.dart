...
class _WasteManagementPageState extends State<WasteManagementPage> {
        ...
        if (mounted) {
          final shouldCreate = await showDialog<bool>(
            ...
            builder: (context) => AlertDialog(
              ...
              title: const Text('Create Waste Tracking Table?',
          ...
          try {
            ...
            if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text(
                      'Failed to create waste tracking table. Please contact your administrator.'),
    ...
    setState(() {
      _filteredRecords = _wasteRecords.where((record) {
        ...
        if (record['inventory'] is Map) {
          itemName =
              record['inventory']['item_name']?.toString().toLowerCase() ?? '';
          category =
              record['inventory']['category']?.toString().toLowerCase() ?? '';
        }
  ...
  Future<void> _selectDate(BuildContext context, bool isStart) async {
    ...
    final DateTime firstDate = isStart
        ...
        : _startDate;
    final DateTime lastDate = isStart ? _endDate : DateTime.now();
  ...
  void _showAddWasteDialog(BuildContext context) {
        ...
        SnackBar(
          content: Row(
            children: [
              ...
              const Expanded(
                child: Text(
                    'You need to add inventory items before recording waste.'),
              ),
              TextButton(
                onPressed: () {
    ...
    Map<String, dynamic>? selectedItem =
        _inventoryItems.isNotEmpty ? _inventoryItems[0] : null;
    double quantity = 1.0;
    double cost = selectedItem != null
        ? (selectedItem['price_per_unit'] as num?)?.toDouble() ?? 0
        : 0;
  ...
  Future<void> _saveWasteRecord({
    ...
    try {
      ...
      await _supabase.from('inventory_waste').insert({
      ...
      try {
        await _supabase.from('inventory_transactions').insert({
          ...
          'notes':
              'Waste: $reason${notes != null && notes.isNotEmpty ? ' - $notes' : ''}',
          'user_id': user.id,
          'restaurant_id': restaurantId,
        });
      } catch (e) {
