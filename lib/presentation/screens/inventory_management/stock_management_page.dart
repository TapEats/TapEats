...
class StockManagementPage extends StatefulWidget {
  ...
  const StockManagementPage({
    super.key,
    this.selectedIndex =
        10, // Default to inventory tab index - adjust this to match your app's navbar structure
  });
...
class _StockManagementPageState extends State<StockManagementPage> {
  ...
  Future<void> _fetchInventoryItems() async {
    ...
    try {
      ...
      final inventoryResult = await _supabase.from('inventory').select('''
            inventory_id,
            item_name,
            category,
            quantity,
            ...
            Expanded(
              ...
              child: Text(
                item['price_per_unit'] != null
                    ? (item['price_per_unit'] as num).toStringAsFixed(2)
                    : '—',
                style: const TextStyle(
                  color: Colors.white,
                  fontSize: 14,
  ...
  void _showAddEditItemModal(BuildContext context,
    ...
    final minQuantityController = TextEditingController(
        text: item?['min_quantity'] != null
            ? (item!['min_quantity'] as num).toString()
            : '');
    final unitController = TextEditingController(text: item?['unit'] ?? '');
    final priceController = TextEditingController(
        text: item?['price_per_unit'] != null
            ? (item!['price_per_unit'] as num).toString()
            : '');
