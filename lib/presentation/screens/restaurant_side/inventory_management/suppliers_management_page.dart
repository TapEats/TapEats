...
class _SuppliersManagementPageState extends State<SuppliersManagementPage> {
  ...
  void _filterSuppliers() {
    ...
    setState(() {
      _filteredSuppliers = _suppliers.where((supplier) {
        ...
        final companyName = supplier['company_name']?.toString().toLowerCase() ?? '';
            ...
            SingleChildScrollView(
              ...
              child: Row(
                children: _productTypes.map((type) => 
                  _buildProductTypeChip(type, _selectedProductType == type)
                ).toList(),
              ),
            ),
  ...
  void _showAddEditSupplierModal(BuildContext context, {Map<String, dynamic>? supplier}) {
    // Controllers for the form fields
    final nameController = TextEditingController(text: supplier?['name'] ?? '');
    final companyNameController = TextEditingController(text: supplier?['company_name'] ?? '');
    final contactController = TextEditingController(text: supplier?['contact'] ?? '');
    final emailController = TextEditingController(text: supplier?['email'] ?? '');
    final addressController = TextEditingController(text: supplier?['address'] ?? '');
    final productTypeController = TextEditingController(text: supplier?['product_type'] ?? '');
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(supplierId != null ? 'Supplier updated successfully' : 'Supplier added successfully'),
